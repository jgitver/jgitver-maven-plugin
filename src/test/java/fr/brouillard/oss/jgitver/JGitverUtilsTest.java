/**
 * Copyright (C) 2019 Marco Jorge [https://github.com/marcobjorge/clock/tree/master]
 * (marcobjorge@gmail.com)
 *
...
public class JGitverUtilsTest {
  ...
  public void normalizeSystemPropertyNameTest() {
    ...
    Assert.assertEquals(
        ...
        "A__a___9_",
        JGitverUtils.normalizeSystemPropertyName("A%$a#%$9_"));
    Assert.assertEquals(
        "Compliant names can start and finish with underscore",
        "_A__a___9_",
        JGitverUtils.normalizeSystemPropertyName("_A%$a#%$9_"));
    Assert.assertEquals(
        "Compliant names cannot start with a digit",
        "__A__a___9_",
        JGitverUtils.normalizeSystemPropertyName("9_A%$a#%$9_"));
    Assert.assertEquals(
        "Multiple initial digits cannot be collapsed",
        "_9A__a___9_",
        JGitverUtils.normalizeSystemPropertyName("99A%$a#%$9_"));
  }
  ...
  public void getPropertyTest() {
    ...
    userProperties
        .entrySet()
        .forEach(e -> systemProperties.put("env." + e.getKey(), "cannot be picked up"));
    ...
    Assert.assertEquals(
        ...
        "priority",
        JGitverUtils.getProperty(mavenSession, "priority.1", logger).get());
    Assert.assertEquals(
        "Normalized match on user properties",
        "priority",
        JGitverUtils.getProperty(mavenSession, "priority.2", logger).get());

    Assert.assertEquals(
        "Direct match on system properties as env property",
        "priority",
        JGitverUtils.getProperty(mavenSession, "priority.3", logger).get());

    Assert.assertEquals(
        "Normalized match on system properties as env property",
        "priority",
        JGitverUtils.getProperty(mavenSession, "priority.4", logger).get());

    Assert.assertEquals(
        "Direct match on system properties as system property",
        "priority",
        JGitverUtils.getProperty(mavenSession, "priority.5", logger).get());
