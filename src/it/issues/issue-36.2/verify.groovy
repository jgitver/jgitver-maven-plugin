...
def versionChanges = buildLog.findAll { it =~ /fr.brouillard.oss.it::issue-36::0 -> 1.0.1-dirty-SNAPSHOT/ }
...
File installedPomFile = new File(localRepositoryPath, "fr/brouillard/oss/it/issue-36/1.0.1-dirty-SNAPSHOT/issue-36-1.0.1-dirty-SNAPSHOT.pom")
log.println( "installedPomFile: " + installedPomFile.isFile() + " " + installedPomFile )
assert installedPomFile.isFile()
def installedPomFileVersion = installedPomFile.readLines().findAll { it =~ /<version>1.0.1-dirty-SNAPSHOT<\/version>/ }
