...
def versionChanges = buildLog.findAll { it =~ /fr.brouillard.oss.it::user-defined-tag-version-pattern::0 -> 2.0.0/ }
...
File installedPomFile = new File(localRepositoryPath, "fr/brouillard/oss/it/user-defined-tag-version-pattern/2.0.0/user-defined-tag-version-pattern-2.0.0.pom")
log.println( "installedPomFile: " + installedPomFile.isFile() + " " + installedPomFile )
assert installedPomFile.isFile()
def installedPomFileVersion = installedPomFile.readLines().findAll { it =~ /<version>2.0.0<\/version>/ }
